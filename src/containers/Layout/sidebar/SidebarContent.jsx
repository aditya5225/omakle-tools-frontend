...
import SidebarBox from './customContent/SidebarBox';
import SidebarGuestBluBox from './customContent/sidebarGuestBluBox';
import IpayDlvryDrvr from '../../../assets/images/Delivery-solution.png';
import IpayDlvrysrvcs from '../../../assets/images/buss-pos-2.jpg';
...
class SidebarContent extends Component {
  ...
  renderLogoutModal = () => {
    return (
      <Modal isOpen={this.state.logoutModal} toggle={this.toggleLogoutModal} className="modal-body" centered={true}>
        <form className="theme-form" noValidate="" >
          <ModalHeader toggle={this.toggleLogoutModal}>Logout</ModalHeader>
          <ModalBody style={{ textAlign: "center" }}>
            <h5>Are you sure you want to logout ?</h5>
            <div style={{ marginTop: "1rem" }}>
              <Button color="secondary" className="square" size="sm" style={{ margin: "0 0.7rem" }}>
                Not, now
              </Button>
              <Button color="primary" className="square" size="sm" style={{ margin: "0 0.7rem" }} onClick={() => this.props.history.push("/account/logout")}>
                Yes, please
              </Button>
            </div>
          </ModalBody>
    ...
    return (
      <div className="sidebar__content">
        ...
        <ul className="sidebar__block">
          <SidebarContentList sidebarCollapse={sidebarCollapse} hideSidebar={this.hideSidebar} />
          {/* <SidebarBox
            boxHeading='DELIVERY SOLUTION'
            boxContent='Deliver meals, groceries etc in 24hrs'
            boxImage={IpayDlvryDrvr}
            boxLink='#'
            onClick={() => window.open("https://ipaypro.co/delivery-solution/order", "_blank")}
            sidebarCollapse={sidebarCollapse}
            collapseSidebarImage={IpayDlvryDrvr}
          />
          {
            this.props.merchant.business_category == "Hospitality" && <SidebarBox
              boxHeading='DELIVERY PARTNERS'
              boxContent='Connect your delivery partners'
              boxImage={IpayDlvrysrvcs}
              boxLink='/delivery-partners'
              sidebarCollapse={sidebarCollapse}
              collapseSidebarImage={deliveryPartner}

            />
          } */}


          {/* <SidebarGuestBluBox sidebarCollapse={sidebarCollapse} newLink='true'
                    />*/}

          <ul className="sidebar__block p-0 " style={{ border: "1px solid #eff1f5" }}>
            <li className="sidebar__link" onClick={this.toggleLogoutModal}>
              <span className={`sidebar__link-icon lnr lnr-exit`} />
              <p className="sidebar__link-title">
                Log Out
              </p>
            </li>
            {/* <SidebarLink title="Log Out" icon="exit" onClick={this.toggleLogoutModal} /> */}
          </ul>
          {/* <SidebarLink
